...
function stockin(){
    ...
    /*  let expiration_date = document.getElementById("expiration_date").value;
     ...
     /* }else if(expiration_date.length == 0 || expiration_date.replace(/^\s+|\s+$/g, "").length == 0){
          ...
          $("#cost_price").focus();
          return;
     }else if(parseInt(sales_price) < 0){
          alert("Cost price cannot be lesser than 0!");
          $("#cost_price").focus();
          return;
     }else{
          $.ajax({
               ...
               data : {posted_by:posted_by, store:store, /* supplier:supplier, */ vendor:vendor, invoice_number:invoice_number, item_id:item_id, quantity:quantity, cost_price:cost_price, sales_price:sales_price/* pack_price:pack_price, pack_size:pack_size, wholesale_price:wholesale_price, wholesale_pack:wholesale_pack,  expiration_date:expiration_date*/},
               beforeSend : function(){
...
function getItems(item_name){
     ...
     if(!loan){
          alert("No loan product selected! Kindly selet a loan product to proceed");
          $("#item").focus();
          return;
     }else{
          if(item.length >= 3){
               if(item){
                    $.ajax({
                         ...
                         data : {item:item, loan:loan, customer:customer},
                         beforeSend : function(){
...
function completeApplication(){
     ...
     let asset = document.getElementById("asset")?.value || 0; // Optional asset field
...
function clientPayment(){
 ...
 // prinit receipt for customer deposit
function printPaymentReceipt(invoice){
     window.open("../controller/loan_payment_receipt.php?receipt="+invoice)
     return false;
 
}

//dispense asset
function dispenseAsset(loan, asset){
     let confirmDis = confirm("Are you sure want to dispense this asset?", "");
     if(confirmDis){
          $.ajax({
               type : "POST",
               url : "../controller/dispense_asset.php",
               data : {loan:loan, asset:asset},
               beforeSend : function(){
                    $("#disburse").html("<div class='processing'><div class='loader'></div></div>");
               },
               success : function(response){
